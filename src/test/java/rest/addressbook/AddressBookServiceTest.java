...
public class AddressBookServiceTest {
	...
	public void serviceIsAlive() throws IOException {
		...
		//////////////////////////////////////////////////////////////////////
        //Test if is idempotente and safe (server doesn't change)
        Client dummyClient = ClientBuilder.newClient();
        Response dummyResponse = client.target("http://localhost:8282/contacts")
            .request().get();
        assertEquals(200, dummyResponse.getStatus());
				List<Person> l2 = dummyResponse.readEntity(AddressBook.class).getPersonList();
        assertEquals(l1, l2);
	}
	...
	public void createUser() throws IOException {
       ...
       assertEquals(201, dummyResponse.getStatus());
       //Chek not idempotent and safe
       assertNotEquals(juanURI, dummyResponse.getLocation());
       Person dummyJuan = dummyResponse.readEntity(Person.class);
       assertNotEquals(1, dummyJuan.getId());

       //Something change == post is not idempotent
	...
	public void createUsers() throws IOException {
		...
		// test that it is safe and idempotent
		//////////////////////////////////////////////////////////////////////
        //Same test that isAlive();

        Response dummyResponse = client.target("http://localhost:8282/contacts/person/3")
            .request(MediaType.APPLICATION_JSON).get();
        assertEquals(response.getStatus(), dummyResponse.getStatus());
        assertEquals(response.getMediaType(), dummyResponse.getMediaType());
        assertEquals(mariaUpdated.getName(), mariaUpdated.getName());
        assertEquals(mariaUpdated.getId(), mariaUpdated.getId());
        assertEquals(mariaURI, mariaUpdated.getHref());
	...
	public void listUsers() throws IOException {
		...
		// test that it is not safe and not idempotent
		//////////////////////////////////////////////////////////////////////
        //Same test as createUser
        Person dummy = new Person();
        dummy.setName("Dummy");
        List<Person>l1 = addressBookRetrieved.getPersonList();
        Response dummyResponse = client.target("http://localhost:8282/contacts")
            .request(MediaType.APPLICATION_JSON)
            .post(Entity.entity(dummy, MediaType.APPLICATION_JSON));
        dummyResponse = client.target("http://localhost:8282/contacts")
            .request(MediaType.APPLICATION_JSON).get();
				List<Person>l2 = dummyResponse.readEntity(AddressBook.class).getPersonList();
        ...
        response = client.target("http://localhost:8282/contacts")
            .request(MediaType.APPLICATION_JSON).get();
				l1 = response.readEntity(AddressBook.class).getPersonList();
        //Not idempotent
	...
	public void updateUsers() throws IOException {
		...
		//////////////////////////////////////////////////////////////////////
    //Not safe, name changes
    assertNotEquals(juanUpdated.getName(), juan.getName());
  	//Another put with mari
  	//idempotent
  	response = client.target("http://localhost:8282/contacts/person/2")
				.request(MediaType.APPLICATION_JSON)
				.put(Entity.entity(maria, MediaType.APPLICATION_JSON));
	  response = client.target("http://localhost:8282/contacts/person/2")
				.request(MediaType.APPLICATION_JSON)
				.get();
    Person dummy = response.readEntity(Person.class);
        assertEquals(dummy.getName(), mariaRetrieved.getName());
        assertEquals(dummy.getId(), mariaRetrieved.getId());
        assertEquals(dummy.getHref(), mariaRetrieved.getHref());
	}
	...
	public void deleteUsers() throws IOException {
		...
		response = client.target("http://localhost:8282/contacts/person/2")
				.request().delete();
		assertEquals(404, response.getStatus());
        //Not safe beacuase delete content from server
